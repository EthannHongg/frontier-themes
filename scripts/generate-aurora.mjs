#!/usr/bin/env node
/**
 * Generates per-brand aurora background scripts (based on AuroraBg by crlang44).
 * @see https://github.com/crlang44/AuroraBg
 */
import fs from 'fs';
...
function buildAuroraScript(brand, mode) {
...
(function () {
  const CONFIG = {
    ...
    dim: ${dim},
    speed: 1.0,
    minimap: ${isDark ? 0.45 : 0.55},
    sticky: ${isDark ? 0.92 : 0.96},
    paletteA: [${primary.map((n) => n.toFixed(3)).join(', ')}],
    paletteB: [${secondary.map((n) => n.toFixed(3)).join(', ')}],
    paletteC: [${accent.map((n) => n.toFixed(3)).join(', ')}],
    scrim: [${scrimBase.join(', ')}],
    isLight: ${!isDark},
    widgetBg: '${widgetBg}',
  };

  const SCRIPT_ID = '__aurora-bg-${brand.id}-${mode}';
  ...
  function injectStyle() {
    ...
    style.id = '__aurora-style-${brand.id}-${mode}';
    style.textContent = \`
  ...
  function startAurora() {
    ...
    scrim.id = '__aurora-scrim-${brand.id}-${mode}';
    ...
    const FRAG = \`
      ...
      void main(){
        ...
        vec3 base = ${isDark ? 'vec3(0.02,0.03,0.06)' : 'vec3(0.94,0.95,0.98)'};
        col=base+col*aurora*${isDark ? '1.35' : '0.85'};
        float star=pow(hash21(floor(frag*0.5)),60.0);
        col+=star*${isDark ? '0.4' : '0.15'}*vec3(0.8,0.9,1.0);
        col*=1.0-${isDark ? '0.30' : '0.12'}*length(uv*vec2(0.7,1.0));
        col+=dither(frag);
...
const auroraDir = path.join(ROOT, 'aurora');
fs.mkdirSync(auroraDir, { recursive: true });

const allBrands = [...brands.bigtech, ...brands.startups];
for (const brand of allBrands) {
  for (const mode of ['dark', 'light']) {
    const script = buildAuroraScript(brand, mode);
    fs.writeFileSync(path.join(auroraDir, `${brand.id}-${mode}.js`), script);
  }
}

console.log(`Generated ${allBrands.length * 2} aurora scripts in ${auroraDir}`);
